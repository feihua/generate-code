...
`,
	Run: func(c *cobra.Command, args []string) {
		...
		converter := NewJSONToDDL(TableName)
		...
		var path = "generate/sql/mysql"
		var params map[string]interface{}
		params = map[string]interface{}{
			"TableName": converter.TableName,
			"Indexes":   converter.GenerateIndexes(),
			"Columns":   converter.GenerateDDL(),
		}
		Generate(params, "template/sql/mysql_dll.tpl", path)
	},
}
...
func init() {
	...
	// go run main.go sql mysql --tableName test --jsonFile test.json
