...
#include "dao/{{.ModuleName}}/{{.GoName}}/{{.GoName}}_dao.h"
#include <vector>

/**
 * {{.Comment}}服务访问对象类
 * 提供对{{.Comment}}服务进行操作的方法，如创建、删除、更新和查询{{.Comment}}信息
 */
class {{.JavaName}}Service {
private:
    {{.JavaName}}DAO {{.LowerJavaName}}Dao;
...
public:
     ...
     */
    int create{{.JavaName}}(const {{.JavaName}}Dto &{{.LowerJavaName}});
     ...
     * @return 删除操作是否成功
     */
    void delete{{.JavaName}}(const std::vector<int64_t> &id);
     ...
     * @return 更新操作是否成功
     */
    int update{{.JavaName}}(const {{.JavaName}}Dto &{{.LowerJavaName}});
