  ...
  <el-dialog :model-value="dialogUpdateFormVisible" title="更新" style="width: 480px;border-radius: 10px">
    ...
    {{range .TableColumn}}
     ...
     <el-form-item label="{{.ColumnComment}}" prop="{{.JavaName}}">{{if isContain .JavaName "Sort"}}
        <el-input-number v-model="props.updateParam.{{.JavaName}}" placeholder="请输入{{.ColumnComment}}"/>
    {{else if isContain .JavaName "sort"}}
        <el-input-number v-model="props.updateParam.{{.JavaName}}" placeholder="请输入{{.ColumnComment}}"/>
    {{else if isContain .JavaName "status"}}
        <el-select v-model="props.updateParam.{{.JavaName}}" placeholder="请选择状态">
   ...
   {{else if isContain .JavaName "Type"}}
     ...
     {{else if isContain .JavaName "remark"}}
        <el-input v-model="props.updateParam.{{.JavaName}}" type="textarea" placeholder="请输入{{.ColumnComment}}"/>
     {{else}}
        <el-input v-model="props.updateParam.{{.JavaName}}" placeholder="请输入{{.ColumnComment}}"/>
...
const handleEdit = async () => {
  if (props.updateParam) {
    let addResult: IResponse = await update{{.JavaName}}(props.updateParam)
    if (addResult.code === 0) {
